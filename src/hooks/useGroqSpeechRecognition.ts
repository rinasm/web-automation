...
interface GroqSpeechRecognitionHook {
  isListening: boolean
...
export const useGroqSpeechRecognition = (): GroqSpeechRecognitionHook => {
  ...
  const processAudioWithGroq = async (audioBlob: Blob) => {
    try {
      ...
      if (!apiKey) {
        throw new Error('Groq API key not set. Please configure your API key in settings.')
      }

      // Set API key in Electron main process
      await window.electronAPI.invoke('set-groq-key', apiKey)
